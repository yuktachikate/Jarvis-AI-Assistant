...
# Use the small Whisper model for speed
WHISPER_MODEL  = "small.en"
# Audio frame size: 20 ms
FRAME_MS       = 20
FRAME_BYTES    = int(SAMPLE_RATE * FRAME_MS / 1000) * 2
# Silence threshold: 0.5 s
...
def tts_worker():
    """Play TTS output directly from numpy via sounddevice."""
    # pull sample rate from the TTS synthesizer once
    sr = tts_client.synthesizer.output_sample_rate

    while True:
        phrase = tts_queue.get()
        if phrase is None:
            break
        # tts() returns just the waveform array
...
def ask_gpt(prompt: str) -> None:
    """ Stream GPT via gpt-4o-mini, push sentences immediately, then prompt for follow-up."""
