...
def tts_worker():
    while True:
        ...
        tts_client.tts_to_file(text=phrase, file_path=tmp.name, speaker=MALE_SPEAKER)
...
def ask_gpt(prompt: str) -> None:
    """ Stream the GPT response; play placeholder and then sentences as they arrive. """
    # placeholder while waiting
    tts_queue.put("Hang on a minute? Asking my intelligence.")
    ...
    try:
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "and after answering, ask if the user has any more questions."
                    )
                },
        ...
        for part in stream_resp:
            ...
            # whenever we hit a sentence terminator, send that sentence to TTS
            while any(buffer.endswith(end) for end in sentence_enders):
                for end in sentence_enders:
                    if end in buffer:
                        idx = buffer.rfind(end) + 1
                        sentence = buffer[:idx].strip()
                        buffer = buffer[idx:].lstrip()
                        tts_queue.put(sentence)
                        logger.info(f"GPT chunk responded: {sentence}")
                        print(f"[DEBUG] GPT chunk responded: {sentence!r}")
                        break

        # any leftover text
        if buffer.strip():
            tts_queue.put(buffer.strip())
            logger.info(f"GPT final chunk responded: {buffer.strip()}")
            print(f"[DEBUG] GPT final chunk responded: {buffer.strip()!r}")

    except Exception as e:
        error_msg = f"GPT API error: {e}"
...
def transcribe_meeting():
    speech_buf, silent_cnt, input_buf = bytearray(), 0, bytearray()
    while True:
        data = audio_queue.get()
        if data is None:
            break
        input_buf.extend(data)
        while len(input_buf) >= FRAME_BYTES:
            frame, input_buf = input_buf[:FRAME_BYTES], input_buf[FRAME_BYTES:]
